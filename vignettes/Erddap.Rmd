...
uses the `calculate_statistics()` and `calculate_SST_anomaly` functions from the **nms4r** package to update the relevant csv files.
...
GitHub Actions using the [update_oceano.yml](https://github.com/marinebon/cinms/blob/master/.github/workflows/update-oceano.yml) workflow. The workflow calls the R script [render_oceano-modals.R](https://github.com/marinebon/cinms/blob/master/scripts/render_oceano-modals.R), which calls the **nms4r** function `render_all_rmd()` to update the relevant modal windows.
